...
import { getLLMClient, getModelName, getResponseFormat, LLM_PROVIDER } from "../lib/llm-config";
...
export async function dynamicAnalysis(
...
DO NOT repeat the same icon for all subtopics.
    ...
    if (LLM_PROVIDER === 'groq') {
      const groqClient = client as any;
      ...
      res = await openaiClient.chat.completions.create({
        ...
        messages,
        response_format: getResponseFormat(schema)
      });
    }
